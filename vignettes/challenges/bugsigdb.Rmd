...
In some cases, authors also make statements about differential abundance on a higher
taxonomic rank for cases where supposedly all (at least all investigated) taxa of
a lower taxonomic rank display differential abundance. An example would be if both
...
increased abundance in a certain condition, authors would sometimes conclude that
...
for their descendants. For example, given differential abundance on the species
level, this could thus be used to infer differential abundance on the genus level,
and further up the taxonomy. 
